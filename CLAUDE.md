...
- `app/Services/PushNotifier.php`, `PushAlerts.php`, `PushController`: Web Push (minishlink/web-push). VAPID keys come from settings (System → Set up notifications) unless VAPID_* are in .env. `Incident`/`Result` `created` events call `PushAlerts`, which only alerts for records from the last 30 minutes (so imports and backfills stay quiet) and sends after the response
